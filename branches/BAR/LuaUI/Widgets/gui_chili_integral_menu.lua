...
local common_commands, states_commands, strategic_commands, econ_commands, globalCommands, overrides, custom_cmd_actions = include("Configs/integral_menu_commands.lua")
...
--shortcuts
local menuChoices = {
	[1] = { array = n_common, name = "O\nR\nD\nE\nR\n", hotkeyName = "O\nR\nD\nE\nR\n" },
	[2] = { array = n_strategic, name = "T\nA\nC\nT\n", hotkeyName = " S\n T\n R\n A\n T\n E\n G\n I\n C\n", config = strategic_commands, actionName = "epic_chili_integral_menu_tab_factory" },
	[3] = { array = n_econ, name = "E\nC\nO\nN\n", hotkeyName = " E\n C\n O\n O\n N\n", config = econ_commands, actionName = "epic_chili_integral_menu_tab_economy" },
	[4] = { array = n_units, name = "U\nN\n I\nT\nS\n", hotkeyName = "U\nN\n I\nT\nS\n" },
}
...
local function MakeButton(container, cmd, insertItem, index) 
	...
	local gridHotkeyed = not isState and menuChoice ~= 1 and menuChoice ~= 4 
	local text
	local texture
	local countText = ''
	...
	if (options.unitstabhotkey.value and menuChoice == 4 and selectedFac and container.i_am_sp_commands) then
	...
	if menuChoice == 4 and isBuild and buildQueueUnsorted[-cmd.id] then
...
--			button.backgroundColor = {0,0,0,0}
		...
		if (isBuild) then
			button.padding = {3,3,2,2}
		end
		
		local label 
		if (not cmd.onlyTexture and text and text ~= '') or gridHotkeyed or menuChoice == 4 then  --commenting out broke widget
			label = Label:New {
				width="100%";
				height="100%";
				autosize=false;
		...
		if (texture and texture ~= "") then
			image= Image:New {
				width="89%";
				height= (not isBuild) and nil or "89%";
--				bottom = (isBuild) and 0 or nil;
				y="6%";
				x="6%";
				color = color;
...
local function ProcessCommand(cmd) 
	if not cmd.hidden and cmd.id ~= CMD_PAGES then 
		...
		elseif strategic_commands[cmd.id] then
			n_strategic[#n_strategic+1] = cmd
...
local function ManageCommandIcons(useRowSort)
	...
	--update factory data
	if menuChoice == 4 and selectedFac then
	...
	if menuChoice == 4 and selectedFac and #commandRows[numRows] == 0 then
...
--	local useRowSort = (menuChoice == 3 or menuChoice == 4 or menuChoice == 5)
	...
	menuChoices[2].array = n_strategic
	menuChoices[3].array = n_econ
	menuChoices[4].array = n_units
	...
	table.sort(n_strategic, Sort(a,b, strategic_commands))
	...
	table.sort(n_strategic, function(a,b) return strategic_commands[a.id].order < strategic_commands[b.id].order end )
...
function widget:KeyPress(key, modifier, isRepeat)
	...
	if (key == KEYSYMS.SPACE or ((key == KEYSYMS.RALT or key == KEYSYMS.LALT) and options.unitshotkeyaltaswell.value)) and selectedFac and menuChoice == 4 and options.unitshotkeyrequiremeta.value  and options.unitstabhotkey.value then
		Update(true)
	end
end
...
--adds hotkey to the end of the tab caption
...
function widget:GameFrame(n)
	--set progress bar
	if n%6 == 0 then
		if menuChoice == 4 and selectedFac and buildRowButtons[1] and buildRowButtons[1].image then
			local progress
