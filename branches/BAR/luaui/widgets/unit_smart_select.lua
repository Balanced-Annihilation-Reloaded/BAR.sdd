...
function widget:GetInfo()
	return {
		name      = "SmartSelect",
		desc      = "'Shift' add to new units to selection + no buildings filter.  'Ctrl' exclude from selection.  'Space' idle units only",
		author    = "aegis",
		date      = "Jan 2, 2011",
...
-- limit the update interval for performance (else on my old laptop fps went 3 times as low)
...
function widget:Update()
	...
	if (DiffTimers(newUpdate, lastUpdate) < updateInterval) then
		return
	end
