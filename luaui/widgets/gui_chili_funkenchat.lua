...
local newMessageTime = 0
local testConOpen = false
local textHeight 
local function displayMessage(msg, remake)
	...
	if (msg.dup > 1 and not remake) then
		...
		local textbox = WG.Chili.TextBox:New{
			...
			padding = { 0, 0, 0, 0 },
			text = text,
			font = {outlineWidth = 3, outlineWeight = 10, outline = true}
		}
		...
		if not testConOpen then window_console.height = window_console.height + textHeight; window_console:AddChild(scrollpanel1); testConOpen = true end
		if newMessageTime < 400 then newMessageTime = newMessageTime + 100; window_console.height = window_console.height + 13 end
...
-- local function tempShowConsole()
	-- window_console:Resize(screen0.width * 0.30,screen0.height * 0.20)
	-- scrollpanel1:Resize("100%", 16)
	-- window_console:AddChild(scrollpanel1)
	-- local timeAgo = Spring.GetTimer()
	-- while Spring.GetTimer() - timeAgo < 10 do
	-- end
	-- window_console:RemoveChild(scrollpanel1)
	-- window_console:Resize(screen0.width * 0.30,inputsize)
-- end
...
function widget:KeyPress(key, modifier, isRepeat)
	Spring.Echo(key)
	if key == 96 and not consoleOpen then
		window_console:SetPos(screen0.width*0.3,screen0.height*0.25,screen0.width*0.4,screen0.height*0.5); window_console:AddChild(scrollpanel1)
		consoleOpen = true
	elseif key == 96 and consoleOpen then
		window_console:SetPos(screen0.width*0.7-350,20,screen0.width*0.3,45); window_console:RemoveChild(scrollpanel1)
		consoleOpen = false
		testConOpen = false
	end
end
...
function widget:GameFrame(n)
	if newMessageTime > 0 then newMessageTime = newMessageTime - 1 end
	if newMessageTime % 100 == 0 and window_console.height >= 58 then
		window_console.height = window_console.height - textHeight --TODO: replace with number based on text size (textbox.height?)
	end
	if newMessageTime == 0 and #window_console.children > 1 then window_console:RemoveChild(scrollpanel1); testConOpen = false end
	window_console:UpdateClientArea()
end
