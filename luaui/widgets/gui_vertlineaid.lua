...
--/radarlines_glow			-- toggles a faint glow on the line
...
local lineOpacity					= 0.5
local fadeMultiplier				= 1		-- lower value: fades out sooner
local showLineGlow					= true
local desiredDisplayInterval_gbl	= 0.015 --// constant: the rest period (in second) between each update of enemy position.
		...
		if content ~= nil then
			...
			if (dotsEnterLos[unitID] == nil) then
				...
				if content ~= nil then
					...
					local yDifference = camY - surfaceY
					...
					if lineOpacityMultiplier > 0.2 then
						...
						if showLineGlow then
							glLineWidth(10)
							glColor(1, 0.75, 0, .04*lineOpacity*lineOpacityMultiplier)
							glBeginEnd(GL_LINES, function() glVertex(x,surfaceY,z) glVertex(x,y,z) end)
...
function widget:GetConfigData(data)
    savedTable = {}
    savedTable.showLineGlow = showLineGlow		-- lower value: fades out sooner
    return savedTable
end
...
function widget:TextCommand(command)
    if (string.find(command, "radarlines_glow") == 1  and  string.len(command) == 15) then 
		showLineGlow = not showLineGlow
		if showLineGlow then
			Spring.Echo("Radar lines:  Glow enabled")
		else
			Spring.Echo("Radar lines:  Glow disabled")
		end
	end
end
